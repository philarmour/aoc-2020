...
const inArr = input.split(', ').map((x) => +x);

const findCompliments = (amount) => {
  const complArr = inArr.map((i) => amount - i);
  const filteredArray = complArr.filter((value) => inArr.includes(value));
  return filteredArray;
};

const twoValues = findCompliments(2020);
console.log(
  `twoValues: ${twoValues.length}: ${twoValues[0]} * ${twoValues[1]} = ${
    twoValues[0] * twoValues[1]
  }`
);

const threeValuesUnfiltered = inArr.map((i) => {
  const newRemainder = 2020 - i;
  // console.log('calling findCompliments with: ', newRemainder);
  const complimentsArr = findCompliments(newRemainder);
  if (complimentsArr.length > 0) {
    console.log(`found compliments: `, complimentsArr);
    complimentsArr.push(i);
    return complimentsArr;
  } else return false;
});

const threeValueSets = threeValuesUnfiltered.filter((i) => i);
// just need one
const threeValues = threeValueSets[0];
console.log(threeValues);
console.log(
  `threeValues: ${threeValues.length}: ${threeValues[0]} * ${
    threeValues[1]
  } * ${threeValues[2]} = ${threeValues[0] * threeValues[1] * threeValues[2]}`
);
